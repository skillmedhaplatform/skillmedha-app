...
export default function MobileJobOpenings() {
  ...
  const todayCount = (JOBS || []).filter(isPostedToday).length;
  const totalApplied = student?.appliedJobs?.length || optimisticAppliedIds.length;
        ...
        {((listFilter === "all" && pagination) || listFilter === "applied") && (
          <div className="flex flex-col gap-3 pt-3 px-3 pb-3 border-t border-[#e2e8f0] bg-white shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] relative z-10">
            ...
            <div className="flex items-center justify-center gap-1 flex-wrap">
              ...
              {Array.from(
                { length: listFilter === "all" ? pagination.totalPages : appliedTotalPages },
                (_, i) => {
                  const pageNum = i + 1;
                  const currentPage = listFilter === "all" ? pagination.currentPage : appliedPage;
                  const totalPages = listFilter === "all" ? pagination.totalPages : appliedTotalPages;
              ...
              <Button
                ...
                disabled={
                  listFilter === "all"
                    ...
                    : appliedPage === appliedTotalPages || appliedTotalDocs === 0
                }
                onClick={() => {
                  if (listFilter === "all") {
                    const params = new URLSearchParams(searchParams);
