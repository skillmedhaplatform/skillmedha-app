...
/**
 * Template40 — "Stat Header"
 * A header followed by a row of small "stat" tiles (years of experience,
 * roles held, projects shipped) — the dashboard-card motif some modern
 * builders use to put a candidate's scale of experience front and
 * center before the reader even reaches Experience.
 */
...
const S = {
  ...
  header: { display: "flex", alignItems: "center", gap: "18px", padding: "28px 30px 22px" },
  photo: { height: "80px", width: "80px", borderRadius: "50%", objectFit: "cover", flexShrink: 0, border: "3px solid #f1f5f9" },
  monogram: (accent) => ({ height: "80px", width: "80px", borderRadius: "50%", backgroundColor: withAlpha(accent, 0.12), color: accent, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "26px", fontWeight: 700, flexShrink: 0 }),
  name: { margin: 0, fontSize: "24px", fontWeight: 800, color: "#111827" },
  ...
  statRow: (accent) => ({ margin: "0 30px 24px", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: "10px" }),
  statTile: (accent) => ({ borderRadius: "10px", padding: "12px 14px", backgroundColor: withAlpha(accent, 0.06), border: `1px solid ${withAlpha(accent, 0.15)}` }),
  statValue: (accent) => ({ margin: 0, fontSize: "20px", fontWeight: 800, color: accent, lineHeight: 1 }),
  statLabel: { margin: "3px 0 0", fontSize: "11px", fontWeight: 600, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em" },
...
const Template40 = ({ resumeTemplateRef, activeSection, isGeneratingPdf, accent: accentProp, onAccentChange }) => {
  const {
  ...
  const stats = [
    totalYears ? { value: `${totalYears}+`, label: "Years experience" } : null,
    combinedExperience.length ? { value: combinedExperience.length, label: "Roles held" } : null,
    projectDetails.filter((p) => p?.project).length ? { value: projectDetails.filter((p) => p?.project).length, label: "Projects" } : null,
    skills.filter(Boolean).length ? { value: skills.filter(Boolean).length, label: "Core skills" } : null,
  ].filter(Boolean);
  ...
  return (
    <div style={{ width: "100%", maxWidth: isGeneratingPdf ? "800px" : "100%", margin: "0 auto" }}>
      ...
      <div ref={resumeTemplateRef} style={S.page}>
        ...
        {stats.length > 0 && (
          <div style={S.statRow(accent)}>
            {stats.map((s, i) => (
              <div key={i} style={S.statTile(accent)}>
                <p style={S.statValue(accent)}>{s.value}</p>
