...
export const getTests = createAsyncThunk("/getAllTests", async (args) => {
  ...
  if (!response.data.data.tests.tests) action.nav.replace("/");
...
export const createTests = createAsyncThunk(
  ...
  async ({ values }) => {
    try {
      const { data } = await axios.post(testsUrl + "/addTest", {
        ...values,
        status: "inprogress",
      });
...
export const SendEmailMail = createAsyncThunk(
  ...
  async (args) => {
    try {
      const { data } = await axios.post(testsUrl + "/sendTestAccessMail", {
...
export const DeleteTest = createAsyncThunk(
  ...
  async ({ test, dispatch }) => {
    ...
    try {
      const { data } = await axios.post(testsUrl + "/deleteTest/" + test?._id);
      hideLoading();
...
export const addCategory = createAsyncThunk("/addCategory", async (args) => {
  const { data } = await axios.post(testsUrl + "/addCategory", {
    name: args.name,
    type: args.type,
  });
