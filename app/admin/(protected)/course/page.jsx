...
import { deleteInternship, getAllCourses } from "@/redux/slices/admin/cms/internship";
...
const Page = () => {
  ...
  const data = useSelector((state) => state.adminInternship?.allCourses?.data) || [];
  const companies = useSelector((state) => state.user?.orgs) || [];
  const cursor = useSelector((state) => state.adminInternship.allCourses?.cursor);
  const hasNext = useSelector((state) => state.adminInternship.allCourses?.hasNext);
  ...
  useEffect(() => {
    ...
    fetchCourses(pageFromUrl, limitFromUrl);
    dispatch(getAllOrgs());
  }, []);
  ...
  const handleCardClick = (eachData) => {
    nav.push(`/admin/course/${eachData?._id}`);
  };
  ...
  const getMenuItems = (cardData) => [
    ...
    },
    {
      ...
      onClick: () => {
        ...
        // handleCardClick(cardData);
        dispatch(deleteInternship({ id: cardData?._id, type: "course" }));
      },
    },
  ];
              ...
              >
                <div className={internshipLibStyles.thumbWrapper}>
                  <img
                    ...
                    alt={eachData?.title || "Course cover"}
                    loading="lazy"
                  />
                  {eachData?.difficulty ? (
            ...
            >
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ fontSize: 12, color: "#777", marginTop: 2, marginLeft: 24 }}>
                  Any new branch/department added to this college later will also get this course automatically.
                </div>
              </div>
              <div>
                <Checkbox
                ...
                <div style={{ fontSize: 12, color: "#777", marginTop: 2, marginLeft: 24 }}>
                  Any new student added to this college later will also get this course automatically.
                </div>
              </div>
            </div>
          )}
