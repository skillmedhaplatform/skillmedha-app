...
export const searchMeetingByTopic = createAsyncThunk("/searchMeetingByTopic" , async (args) => {
  try {
    const {type , text , limit , cursor} = args

    const {data} = await axios.get(internshipUrl + `/searchTopics?limit=${limit}&cursor=${cursor}&query=${text}&type=${type}`)
...
export const getAllMeetings = createAsyncThunk(
  ...
  async (payload) => {
    try {
      ...
      const { data } = await axios.get(restUrl + `/getAllMeetings?type=${type}&cursor=${cursor}&limit=${limit}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
export const getOneMeeting  = createAsyncThunk("/getOneMeeting" ,async(args) => {
  try {
      const {id} = args

      const {data} = await axios.post(restUrl + "/getMeetingDetails" , {
        id
      })
...
export const updateMeeting = createAsyncThunk("/updateMeeting" , async(args) => {
  const hide = message.loading("Please wait while updating meeting" , 0)
  try {
    const {id} = args

    const {data} = await axios.post(restUrl + "/updateMeeting/" + id , {
      isCompleted : true
    })
