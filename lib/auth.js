...
export const roleConfig = {
  ...
  tpo: {
    ...
    loginApi: `${process.env.NEXT_PUBLIC_LOGIN_URL}/login`,
    getDashboardRoute: (data) => {
      return `/tpo/myprofile?token=${encodeURIComponent(data.token)}&userId=${encodeURIComponent(data.userID)}`;
    },
    type: 'college',
    storeTokens: (data) => {
      if (typeof window === 'undefined') return;
...
export const authService = {
  ...
  */
  loginByRole: async (role, email, password) => {
    ...
    console.log(`[unified-login] Login successful for role: ${role}. Redirecting to: ${redirectUrl}`);
