...
import {
  MdTextFields, // TEXT
  MdRadioButtonChecked, // SINGLE_CHOICE
  MdCheckBox, // MULTIPLE_CHOICE
  MdCheckCircle, // TRUE_FALSE
  MdAudiotrack, // AUDIO
  MdVideocam, // VIDEO
} from "react-icons/md";
...
const labelOptions = [
  ...
  { name: QUESTION_TYPES.TRUE_FALSE, icon: <MdCheckCircle /> },
  { name: QUESTION_TYPES.AUDIO, icon: <MdAudiotrack /> },
  { name: QUESTION_TYPES.VIDEO, icon: <MdVideocam /> },
];
...
const QuestionEditorUI = () => {
  ...
  useEffect(() => {
    if (!questions || isNewQuestion) return;
    const existing =
      questions?.find((q) => q?._id === questionId || q?.id === questionId) ||
      null;
    if (!existing) return;
  ...
  const handleSubmit = async () => {
    ...
    const { subjectId, topicId, subTopicId, ...updatedPayload } = questionData;
