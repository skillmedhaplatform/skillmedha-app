...
/**
 * Template33 — "Chip Header"
 * Header with name, photo and contact, then an immediate row of rounded
 * skill chips right under the header (skills-forward layout, useful for
 * technical roles where the skill list is the first thing a recruiter
 * scans for), followed by standard chronological sections.
 */
...
const S = {
  ...
  header: { display: "flex", alignItems: "center", gap: "18px", padding: "28px 30px 20px" },
  photo: { height: "84px", width: "84px", borderRadius: "50%", objectFit: "cover", flexShrink: 0, border: "3px solid #f1f5f9" },
  monogram: (accent) => ({ height: "84px", width: "84px", borderRadius: "50%", backgroundColor: withAlpha(accent, 0.12), color: accent, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "28px", fontWeight: 700, flexShrink: 0 }),
  ...
  chipStrip: (accent) => ({ margin: "0 30px 24px", padding: "16px 18px", borderRadius: "12px", backgroundColor: withAlpha(accent, 0.06), display: "flex", flexWrap: "wrap", gap: "8px" }),
  chip: (accent) => ({ fontSize: "12.5px", fontWeight: 700, color: "#ffffff", backgroundColor: accent, padding: "6px 14px", borderRadius: "999px" }),
...
const Template33 = ({ resumeTemplateRef, activeSection, isGeneratingPdf, accent: accentProp, onAccentChange }) => {
  const {
    ...
    <div style={{ width: "100%", maxWidth: isGeneratingPdf ? "800px" : "100%", margin: "0 auto" }}>
      ...
      <div ref={resumeTemplateRef} style={S.page}>
        ...
        {skills.filter(Boolean).length > 0 && (
          <div id="section-Skills" style={{ ...S.chipStrip(accent), ...highlight("Skills") }}>
            {skills.filter(Boolean).map((skill, i) => <span key={i} style={S.chip(accent)}>{skill}</span>)}
          </div>
        )}
