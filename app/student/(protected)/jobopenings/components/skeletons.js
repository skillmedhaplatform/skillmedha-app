      ...
      <div className="h-full w-full">
        ...
        <div className="w-full my-4 flex items-center justify-center">
          {JOB_DETAIL_TABS.map(({ key, label }, i) => (
            <div
              key={key}
              className={`w-1/3 text-center rounded-[0.1rem] border border-solid border-[rgba(128,128,128,0.5)] py-2 font-extrabold cursor-pointer ${i === 0 ? "bg-[#1E69DA] text-white" : "bg-[#1E69DA]/20 text-[#1E69DA]"}`}
              style={{ opacity: 0.35 }}
            >
              {label}
            </div>
