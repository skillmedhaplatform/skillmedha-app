...
export const roleConfig = {
  ...
  company: {
    ...
    loginApi: `${process.env.NEXT_PUBLIC_LOGIN_URL}/login`,
    getDashboardRoute: (data) => {
      return `/company/profile?token=${encodeURIComponent(data.token)}`;
    },
    type: 'company',
    storeTokens: (data) => {
      if (typeof window === 'undefined') return;
...
export const authService = {
  ...
  */
  loginByRole: async (role, email, password) => {
    ...
    // Handle nested data format from /loginStudent endpoint
    if (data && data.success && data.data) {
      data = {
        ...data.data,
        token: data.token || data.cryptoToken,
        userID: data.data._id || data.data.userID || data.userID,
  ...
  forgotPassword: async (role, email) => {
    ...
    const forgotUrl = `${process.env.NEXT_PUBLIC_STUDENT_URL}/forgotStudentPassword`;
